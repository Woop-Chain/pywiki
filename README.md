# Pywiki - WoopChain's python utilities

**This library only supports Python 3.6+**

A Python library for interacting and working the [WoopChain blockchain](https://wikiwoop.com/)
and [related codebases](https://github.com/woop-chain).

[Full documentation is located on Woop's GitBook](https://docs.wikiwoop.com/) (in progress).
...
```
On MacOS:
Make sure you have Python3 installed, and use python3 to install pywiki
```bash
...
## Running tests
Before you can run tests, you need the python dependencies (`make install`), `docker` and `go` installed to quickly run a local blockchain with staking enabled (detailed instructions [here](https://github.com/woop-chain/woop/blob/main/README.md)):
```bash
...
```py
from pywiki.validator import Validator
...
### Transactions
```py
from pywiki import transaction
```
##### Pool
```py
...
```py
from pywiki import staking_structures, staking_signing
```
