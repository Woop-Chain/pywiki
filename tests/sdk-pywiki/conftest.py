...
# private keys
# 1f84c95ac16e6a50f08d44c7bde7aff8742212fda6e4321fde48bf83bef266dc / woo155jp2y76nazx8uw5sa94fr0m4s5aj8e5xm6fu3 (genesis)
# 3c86ac59f6b038f584be1c08fced78d7c71bb55d5655f81714f3cddc82144c65 / woo1ru3p8ff0wsyl7ncsx3vwd5szuze64qz60upg37 (transferred 503)
...
txs = [
    # same shard 503 WOO transfer from woo155jp2y76nazx8uw5sa94fr0m4s5aj8e5xm6fu3 to woo1ru3p8ff0wsyl7ncsx3vwd5szuze64qz60upg37 (0 nonce)
    ...
    # contract creation by woo1ru3p8ff0wsyl7ncsx3vwd5szuze64qz60upg37 (0 nonce)
    ...
    # cross shard transfer by woo1ru3p8ff0wsyl7ncsx3vwd5szuze64qz60upg37 (1 nonce)
...
stxs = [
    # creation of woo155jp2y76nazx8uw5sa94fr0m4s5aj8e5xm6fu3 as validator (1 nonce)
    ...
    # delegation by woo1ru3p8ff0wsyl7ncsx3vwd5szuze64qz60upg37 to woo155jp2y76nazx8uw5sa94fr0m4s5aj8e5xm6fu3 (2 nonce)
...
def _check_connection():
    try:
        ...
        if "error" in metadata:
            pytest.skip(
                f"Error in wikiv2_getNodeMetadata reply: {metadata['error']}",
                allow_module_level = True,
            )
        if "chain-config" not in metadata[ "result" ]:
            pytest.skip(
                "Chain config not found in wikiv2_getNodeMetadata reply",
                allow_module_level = True,
            )
        return metadata
    except Exception as e:
        pytest.skip(
            "Can not connect to local blockchain or bad wikiv2_getNodeMetadata reply",
            allow_module_level = True,
        )
...
def _check_staking_epoch( metadata ):
    ...
    try:
        ...
        if "error" in latest_header:
            pytest.skip(
                f"Error in wikiv2_latestHeader reply: {latest_header['error']}",
                allow_module_level = True,
            )
    except Exception as e:
        pytest.skip(
            "Failed to get wikiv2_latestHeader reply",
            allow_module_level = True
        )
...
def _send_transaction( raw_tx, endpoint ):
    try:
        ...
        if "error" in tx:
            pytest.skip(
                f"Error in wikiv2_sendRawTransaction reply: {tx['error']}",
                allow_module_level = True,
            )
    except Exception as e:
        pytest.skip(
            "Failed to get wikiv2_sendRawTransaction reply",
            allow_module_level = True
        )
...
def _check_transaction( tx_hash, endpoint ):
    try:
        ...
        pytest.skip(
            "Failed to get wikiv2_getTransactionByHash reply",
            allow_module_level = True
        )
...
def _send_staking_transaction( raw_tx, endpoint = endpoint ):
    try:
        ...
        if "error" in staking_tx:
            pytest.skip(
                f"Error in wikiv2_sendRawStakingTransaction reply: {staking_tx['error']}",
                allow_module_level = True,
            )
    except Exception as e:
        pytest.skip(
            "Failed to get wikiv2_sendRawStakingTransaction reply",
            allow_module_level = True,
        )
...
def _check_staking_transaction( stx_hash, endpoint = endpoint ):
    try:
        ...
        pytest.skip(
            "Failed to get wikiv2_getStakingTransactionByHash reply",
            allow_module_level = True,
        )
