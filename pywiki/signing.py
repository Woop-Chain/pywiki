"""
Sign Woop or Ethereum transactions
Woop staking transaction signing is not covered by this module
"""
...
class UnsignedWoopTxData( HashableRLP ):
    """
    Unsigned Woop transaction data
...
class SignedWoopTxData( HashableRLP ):
    """
    Signed Woop transaction data
    Includes `shardID` and `toShardID`
    as the difference against Eth
    """

    fields = UnsignedWoopTxData._meta.fields + (
...
def serialize_transaction( filled_transaction ):
    ...
    if "v" in filled_transaction:
        # https://github.com/woop-chain/woop/blob/f8879f5e0288157bf95ae2898a9a27f0c85ff9ad/core/types/transaction_signing.go#L173
...
def sign_transaction( transaction_dict, private_key ) -> SignedTransaction:
    ...
    filled_transaction = pipe(
        ...
        partial( merge,
                 TRANSACTION_DEFAULTS ),
        chain_id_to_v,
        apply_formatters_to_dict( WOOP_FORMATTERS ),
    )
