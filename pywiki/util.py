...
def get_bls_build_variables():
    ...
    wiki_path = f"{get_gopath()}/src/github.com/woop-chain"
    bls_dir = f"{wiki_path}/bls"
    mcl_dir = f"{wiki_path}/mcl"
    assert os.path.exists( bls_dir ), f"Woop BLS repo not found at {bls_dir}"
    assert os.path.exists( mcl_dir ), f"Woop MCL repo not found at {mcl_dir}"
