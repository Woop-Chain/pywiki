...
Example:
    Below is a demo of how to import, manage keys, and interact with the CLI::
        >>> from pywiki import cli
        >>> cli.single_call("wiki keys add test1")
        ...
        ['test1']
        >>> cli.single_call("wiki keys list", timeout=2)
        'NAME \t\t ADDRESS\n\ntest1 \twoo1aqfeed538xf7n0cfh60tjaeat7yw333pmj6sfu\n'
        >>> cli.get_accounts_keystore()
        {}
...
Example:
    Below is a demo of how to set the CLI binary used by the module::
        ...
        >>> new_path
        '/Users/danielvdm/go/src/github.com/woop-chain/pywiki/bin/test'
        >>> from pywiki import cli
...
ARG_PREFIX = "__PYWIKI_ARG_PREFIX__"
...
def account_keystore_path( value = None ):
    ...
    if "value" not in account_keystore_path.__dict__:
        account_keystore_path.value = "~/.wiki/account-keys"
    if value:
...
def _set_account_keystore_path():
    ...
    binary."""
    response = single_call( "wiki keys location" ).strip()
...
def _make_call_command( command ):
    ...
    if re.match( ".*wiki", command_toks[ 0 ] ):
...
def is_valid_binary( path ):
    """
    :param path: Path to the Woop CLI binary (absolute or relative).
    ...
    try:
        with subprocess.Popen(
            ...
            if not err:
                return False
            return "woop" in err.decode().strip().lower()
    except (
        OSError,
...
def single_call( command, timeout = 60, error_ok = False ):
    ...
    :returns: Decoded string of response from wiki CLI call
...
def download( path = "./bin/wiki", replace = True, verbose = True ):
    ...
    if not os.path.exists( path ) or replace:
        ...
        os.chdir( parent_dir )
        wiki_script_path = os.path.join( parent_dir, "wiki.sh" )
        with open( wiki_script_path, "w", encoding = 'utf8' ) as script_file:
            script_file.write(
                requests.get(
                    "https://raw.githubusercontent.com/woop-chain/go-sdk/master/scripts/wiki.sh"
                ).content.decode()
            )
        os.chmod(
            wiki_script_path,
            os.stat( wiki_script_path ).st_mode | stat.S_IEXEC
        )
        ...
        if verbose:
            subprocess.call( [ wiki_script_path, "-d" ] )
        else:
        ...
        os.rename( os.path.join( parent_dir, "wiki" ), path )
        if same_name_file:
            os.rename(
                os.path.join( parent_dir,
                              ".wiki_tmp" ),
        ...
        if verbose:
            print( f"Saved woop binary to: `{path}`" )
        os.chdir( old_cwd )
    ...
    if sys.platform.startswith( "darwin" ):  # Dynamic linking for darwin
        try:
            ...
            if files_in_parent_dir.intersection( _libs ) == _libs:
                ...
                f"{get_gopath()}/src/github.com/woop-chain/bls"
            ) and os.path.exists(
                f"{get_gopath()}/src/github.com/woop-chain/mcl"
            ):
                env.update( get_bls_build_variables() )
            else:
                raise RuntimeWarning(
                    f"Could not get environment for downloaded wiki CLI at `{path}`"
                )
